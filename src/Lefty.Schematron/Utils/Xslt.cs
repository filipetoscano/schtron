...
internal static class Xslt
{
    ...
    /// </summary>
    internal static string Transpile( string schema, OutputFormat format )
    {
        var pipeline = format == OutputFormat.Xslt3 ? _xslt3.Value : _xslt2.Value;

        try
        {
            return Apply( schema.AsSource(), pipeline );
        }
    ...
    /// </summary>
    internal static XsltExecutable Compile( string xslt )
    {
        try
        {
    ...
    /// </summary>
    internal static string Run( string document, XsltExecutable xslt )
    {
        try
        {
            return Apply( document.AsSource(), xslt );
        }
        catch ( Exception ex ) when ( ex is not SchematronException )
        {
    ...
    /// </summary>
    private static string Apply( Source src, XsltExecutable xslt )
    {
